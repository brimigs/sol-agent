...
const mockCtx = {
    identity: { sandboxId: SANDBOX_ID },
    agentClient: { exec: mockExec, writeFile: mockWriteFile },
    config: {},
    db: {},
    inference: {},
};
...
describe("exec tool – forbidden commands (self-preservation guard)", () => {
    ...
    it("blocks: DELETE FROM turns", async () => {
        const result = await execTool.execute({ command: "DELETE FROM turns WHERE id = '1'" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockExec).not.toHaveBeenCalled();
    });
    ...
    it("blocks: sandbox_delete with own sandboxId", async () => {
        const result = await execTool.execute({ command: `sandbox_delete ${SANDBOX_ID}` }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockExec).not.toHaveBeenCalled();
    });
});
describe("exec tool – allowed commands", () => {
    ...
    it("allows: sandbox_delete with a different sandboxId", async () => {
        const result = await execTool.execute({ command: "sandbox_delete other-sandbox-xyz" }, mockCtx);
        expect(result).not.toMatch(/^Blocked:/);
...
describe("write_file tool – protected path guard (isProtectedFile)", () => {
    it("blocks: wallet.json (direct name)", async () => {
        const result = await writeFileTool.execute({ path: "wallet.json", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
    ...
    it("blocks: /root/.sol-agent/wallet.json (absolute path)", async () => {
        const result = await writeFileTool.execute({ path: "/root/.sol-agent/wallet.json", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockWriteFile).not.toHaveBeenCalled();
    });
    it("blocks: state.db", async () => {
        const result = await writeFileTool.execute({ path: "state.db", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockWriteFile).not.toHaveBeenCalled();
    });
    it("blocks: constitution.md", async () => {
        const result = await writeFileTool.execute({ path: "constitution.md", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockWriteFile).not.toHaveBeenCalled();
    });
    it("blocks: path containing .ssh", async () => {
        const result = await writeFileTool.execute({ path: "/root/.ssh/authorized_keys", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockWriteFile).not.toHaveBeenCalled();
    });
    it("allows: /tmp/output.txt (unprotected path)", async () => {
        const result = await writeFileTool.execute({ path: "/tmp/output.txt", content: "safe" }, mockCtx);
        expect(result).not.toMatch(/^Blocked:/);
        expect(mockWriteFile).toHaveBeenCalledWith("/tmp/output.txt", "safe");
    });
    it("allows: /root/myapp/index.js (unprotected path)", async () => {
        const result = await writeFileTool.execute({ path: "/root/myapp/index.js", content: "console.log('hi')" }, mockCtx);
        expect(result).not.toMatch(/^Blocked:/);
    ...
    it("allows: a file inside a directory that happens to be named 'state.db'", async () => {
        ...
        const result = await writeFileTool.execute({ path: "/tmp/state.db/output.txt", content: "data" }, mockCtx);
        expect(result).not.toMatch(/^Blocked:/);
        expect(mockWriteFile).toHaveBeenCalledWith("/tmp/state.db/output.txt", "data");
    });
    it("still blocks the actual state.db file", async () => {
        const result = await writeFileTool.execute({ path: "/root/.sol-agent/state.db", content: "evil" }, mockCtx);
        expect(result).toMatch(/^Blocked:/);
        expect(mockWriteFile).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=tools-guard.test.js.map