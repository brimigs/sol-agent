    ...
    return [
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const amount = args.amount_usdc;
                if (amount > balance / 2) {
                    return `Blocked: Cannot transfer more than half your USDC (${balance.toFixed(4)} USDC). Self-preservation.`;
                }
                const result = await transferUsdc(ctx.identity.keypair, args.to_address, amount, ctx.config.solanaNetwork, ctx.config.solanaRpcUrl);
                if (!result.success)
        ...
        {
            name: "x402_fetch",
            description: "Fetch a URL using the x402 HTTP payment protocol on Solana. If the server responds with HTTP 402, automatically builds and signs a USDC SPL token transfer transaction (the server broadcasts it), then retries with the X-PAYMENT header. Compatible with all x402-compliant Solana services.",
            category: "solana",
            parameters: {
                type: "object",
                properties: {
        ...
        {
            name: "probe_x402",
            description: "Check if a URL requires x402 payment. Returns the payment requirements (amount, token, network) if it does, or confirms it is free to access.",
            category: "solana",
            parameters: {
                type: "object",
                properties: {
            ...
            execute: async (args) => {
                ...
                const requirements = await probeX402(args.url);
                if (!requirements) {
        ...
        },
        {
            ...
            description: "ALWAYS call this before pull_upstream. Shows every upstream commit with its full diff.",
            category: "self_mod",
            parameters: { type: "object", properties: {} },
...
========================`;
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return `Entered low-compute mode. Model switched to cheaper option. Reason: ${args.reason || "manual"}`;
            },
        },
        {
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const { createSkill } = await import("../skills/registry.js");
                const skill = await createSkill(args.name, args.description, args.instructions, ctx.config.skillsDir || "~/.sol-agent/skills", ctx.db, ctx.agentClient);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await removeSkill(args.name, ctx.db, ctx.agentClient, ctx.config.skillsDir || "~/.sol-agent/skills", args.delete_files || false);
        ...
        // ── Git Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const repoPath = args.path || "~/.sol-agent";
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return await gitDiff(ctx.agentClient, args.path || "~/.sol-agent", args.staged || false);
            },
        },
        {
            name: "git_commit",
            ...
            execute: async (args, ctx) => {
                ...
                return await gitCommit(ctx.agentClient, args.path || "~/.sol-agent", args.message, args.add_all !== false);
            },
        },
        {
            name: "git_log",
            ...
            execute: async (args, ctx) => {
                ...
                const entries = await gitLog(ctx.agentClient, args.path || "~/.sol-agent", args.limit || 10);
                if (entries.length === 0)
        ...
        // ── Solana Registry Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const network = (args.network || ctx.config.solanaNetwork || "mainnet-beta");
                const entry = await registerAgent(ctx.identity.keypair, ctx.config.name, args.agent_uri, network, ctx.db, ctx.config.solanaRpcUrl);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const sig = await leaveFeedback(ctx.identity.keypair, args.agent_address, args.score, args.comment, network, ctx.db, ctx.config.solanaRpcUrl);
