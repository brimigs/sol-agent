...
export function loadHeartbeatConfig(configPath) {
    ...
    try {
        ...
        const parsedEntries = (parsed.entries || []).map((e) => ({
            name: e.name,
            schedule: e.schedule,
            task: e.task,
        ...
        return {
            entries,
            defaultIntervalMs: parsed.defaultIntervalMs || DEFAULT_HEARTBEAT_CONFIG.defaultIntervalMs,
            lowComputeMultiplier: parsed.lowComputeMultiplier ||
...
function mergeWithDefaults(entries) {
    ...
    const orderedDefaultEntries = defaults.map((defaultEntry) => mergedByName.get(defaultEntry.name) || defaultEntry);
